...
An end-to-end implementation of the four protocol verbs running a verifiable election, followed by twelve simulated attack scenarios. This
is the smallest real run of the kernel loop: the Heeley Bank allotment election (the
[`club-agm`](../scenarios/club-agm.json) scenario, [§15](https://glassontin.github.io/civic-kernel/#s15 "Adoption begins with immediate practical utility: discovering unclaimed entitlements privately and running verifiable club elections.")'s "wedge two"). It emits a
...
- **`unlinkable: true` means the anonymity set is the roster, and the cost is linear in
  ...
  **8.4 kB** of a **13.4 kB** ballot and takes **~0.7 s** to verify; a whole-transcript
  ...
  scale, which is the only scale this prototype claims, and [§4](https://glassontin.github.io/civic-kernel/#s4 "A personal device can submit ballots and audit the transparency log; users without smartphones participate with equal validity through assisted channels.")'s kilobyte-payload
  requirement (T14) survives *here*, at sixty. It does not survive a national roster, and
  no amount of engineering turns an O(n) proof into an O(1) one. This is precisely why [§13](https://glassontin.github.io/civic-kernel/#s13 "Many underlying components exist in isolation; the goal is integrating them into an accessible, user-verifiable client application.")
  ...
  other line of the protocol, which is the claim [§13](https://glassontin.github.io/civic-kernel/#s13 "Many underlying components exist in isolation; the goal is integrating them into an accessible, user-verifiable client application.") makes, and this rung is the first
  evidence for it.
