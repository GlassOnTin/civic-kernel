...
_Generated by `tools/triage.py` over `triage/cases.json` (39 cases: 30 accepted, 9 refused)._
...
| 5 | automated-welfare-reasons | 6 | 3 × 2 | record_absent | **encoded** (`automated-welfare-reasons`) |
| 6 | exam-algorithm-downgrade ⚠ legible-not-remedied | 6 | 3 × 2 | record_absent, pattern_of_lawful_acts | **encoded** (`exam-algorithm-downgrade`) |
| 7 | family-contact-order-unenforced ⚠ legible-not-remedied | 6 | 3 × 2 | pattern_of_lawful_acts, inaction_no_artifact | **candidate** |
| 8 | family-court-secrecy ⚠ legible-not-remedied | 6 | 3 × 2 | access_gated, pattern_of_lawful_acts | **candidate** |
| 9 | grenfell-warnings-ignored ⚠ legible-not-remedied | 6 | 3 × 2 | inaction_no_artifact, record_absent | **encoded** (`grenfell-warnings-ignored`) |
| 10 | maternity-harm-downgraded ⚠ legible-not-remedied | 6 | 3 × 2 | pattern_of_lawful_acts, inaction_no_artifact | **encoded** (`maternity-harm-downgraded`) |
| 11 | nhs-data-access-opaque | 6 | 3 × 2 | access_gated, record_absent | **encoded** (`nhs-data-access-opaque`) |
| 12 | procurement-vip-lane ⚠ legible-not-remedied | 6 | 3 × 2 | pattern_of_lawful_acts, record_absent | **encoded** (`procurement-vip-lane`) |
| 13 | regulator-inaction ⚠ legible-not-remedied | 6 | 3 × 2 | pattern_of_lawful_acts, inaction_no_artifact | **encoded** (`regulator-inaction`) |
| 14 | single-justice-procedure-opaque ⚠ legible-not-remedied | 6 | 3 × 2 | documentary_gate, record_absent | **encoded** (`single-justice-procedure-opaque`) |
| 15 | carers-allowance-overpayment ⚠ legible-not-remedied | 4 | 2 × 2 | inaction_no_artifact, record_absent | **encoded** (`carers-allowance-overpayment`) |
| 16 | citizenship-deprivation-basis ⚠ legible-not-remedied | 4 | 2 × 2 | record_absent | **encoded** (`citizenship-deprivation-basis`) |
| 17 | criminal-disclosure-failure ⚠ legible-not-remedied | 4 | 2 × 2 | access_gated, record_absent | **encoded** (`criminal-disclosure-failure`) |
| 18 | dwp-death-reviews-secret ⚠ legible-not-remedied | 4 | 2 × 2 | access_gated, record_absent | **encoded** (`dwp-death-reviews-secret`) |
| 19 | facial-recognition-no-basis | 4 | 2 × 2 | record_absent | **encoded** (`facial-recognition-no-basis`) |
| 20 | ministerial-standards-unenforced ⚠ legible-not-remedied | 4 | 2 × 2 | inaction_no_artifact | **encoded** (`ministerial-standards-unenforced`) |
| 21 | ofsted-single-word-judgment ⚠ legible-not-remedied | 4 | 2 × 2 | record_absent | **encoded** (`ofsted-single-word-judgment`) |
| 22 | police-vetting-ignored ⚠ legible-not-remedied | 4 | 2 × 2 | inaction_no_artifact, record_absent | **encoded** (`police-vetting-ignored`) |
| 23 | protest-conditions-no-basis | 4 | 2 × 2 | record_absent | **encoded** (`protest-conditions-no-basis`) |
| 24 | removal-without-legible-warrant | 4 | 2 × 2 | record_absent | **encoded** (`removal-without-legible-warrant`) |
| 25 | send-ehcp-refusals ⚠ legible-not-remedied | 4 | 2 × 2 | pattern_of_lawful_acts, inaction_no_artifact | **encoded** (`send-ehcp-refusals`) |
| 26 | us-civil-asset-forfeiture ⚠ legible-not-remedied | 4 | 2 × 2 | record_absent, documentary_gate | **encoded** (`us-civil-asset-forfeiture`) |
| 27 | voter-id | 4 | 2 × 2 | documentary_gate | **encoded** (`voter-id`) |
| 28 | drc-medical-detention ⚠ legible-not-remedied | 3 | 3 × 1 | record_absent | **encoded** (`drc-medical-detention`) |
| 29 | government-by-whatsapp ⚠ legible-not-remedied | 3 | 3 × 1 | record_destroyed | **encoded** (`government-by-whatsapp`) |
| 30 | infected-blood-coverup ⚠ legible-not-remedied | 3 | 3 × 1 | record_destroyed | **encoded** (`infected-blood-coverup`) |

### Recommended to encode next

The highest-leverage cases not yet built as scenarios:

- **family-contact-order-unenforced** (score 6) — A court orders contact between a child and a parent; the order is breached serially; each enforcement application is heard alone, months later, and almost none ends in any sanction. No record aggregates the breaches, so the passage of time quietly becomes the ruling.  
  _Lead: Fathers4Justice campaign territory (user-flagged); Family Justice Review; HMCTS enforcement statistics_
- **family-court-secrecy** (score 6) — Family courts decide childhoods in closed hearings: judgments largely unpublished, parties silenced by contempt rules, no way for anyone to check a pattern across cases — while the children's privacy that justifies the sealing is real, and cuts against the obvious fix.  
  _Lead: Fathers4Justice campaign territory (user-flagged); President of the Family Division's Transparency Review (2021); reporting pilots from 2023_
