...
## The problem ([§1](https://glassontin.github.io/civic-kernel/#s1 "The goal: produce decisions that participants accept as legitimate, even when all parties, including the operators, have incentives to manipulate outcomes."))
...
## Goals, in priority order ([§1](https://glassontin.github.io/civic-kernel/#s1 "The goal: produce decisions that participants accept as legitimate, even when all parties, including the operators, have incentives to manipulate outcomes."))
...
## The threats ([§2](https://glassontin.github.io/civic-kernel/#s2 "Identify every failure mode before designing the system: fourteen attacks, each with a designated countermeasure, and two left unaddressed to avoid creating a centralized arbiter of truth."))
...
| | threat | answer |
|---|---|---|
| T1 | state capture | nothing worth capturing: thin kernel, distributed operators, forkable with history ([§3](https://glassontin.github.io/civic-kernel/#s3 "The system consists of four basic functions: proving eligibility, conducting verifiable votes, enforcing human rights constraints, and maintaining an append-only log."), [§5](https://glassontin.github.io/civic-kernel/#s5 "Rules can be amended through staged and reversible procedures, while fundamental equality of standing remains protected against arbitrary repeal.")) |
| T2 | vendor capture | open protocol, open clients, no remote attestation ([§1](https://glassontin.github.io/civic-kernel/#s1 "The goal: produce decisions that participants accept as legitimate, even when all parties, including the operators, have incentives to manipulate outcomes."), [§4](https://glassontin.github.io/civic-kernel/#s4 "A personal device can submit ballots and audit the transparency log; users without smartphones participate with equal validity through assisted channels.")) |
| T3 | [sybils](README.md#w-sybil) | ceremony-issued credentials; cryptographic uniqueness per decision ([§3.1](https://glassontin.github.io/civic-kernel/#s3-1 "Prove you are on the roster and have not already participated, without revealing your identity or building a personal profile."), [§10](https://glassontin.github.io/civic-kernel/#s10 "Enrolment is a public, witnessed ceremony that counts participants rather than recording identities; duplicate registrations and wrongful exclusions are mathematically visible.")) |
| T4 | coercion & vote buying | [receipt-free](README.md#w-receipt-free) verifiable ballots; silent re-vote, last ballot counts ([§3.2](https://glassontin.github.io/civic-kernel/#s3-2 "Your ballot is sealed before submission and counted without being decrypted; you can verify it was counted, but cannot prove how you voted.")) |
| T5 | device compromise | keys in enclaves, [cast-or-audit](README.md#w-cast-or-audit) challenges, second channel, paper path ([§3.2](https://glassontin.github.io/civic-kernel/#s3-2 "Your ballot is sealed before submission and counted without being decrypted; you can verify it was counted, but cannot prove how you voted."), [§4](https://glassontin.github.io/civic-kernel/#s4 "A personal device can submit ballots and audit the transparency log; users without smartphones participate with equal validity through assisted channels.")) |
| T6 | surveillance chill | [zero-knowledge](README.md#w-zero-knowledge) eligibility, unlinkable pseudonyms, local-first data ([§3.1](https://glassontin.github.io/civic-kernel/#s3-1 "Prove you are on the roster and have not already participated, without revealing your identity or building a personal profile."), [§4](https://glassontin.github.io/civic-kernel/#s4 "A personal device can submit ballots and audit the transparency log; users without smartphones participate with equal validity through assisted channels.")) |
| T7 | majoritarian rights-stripping | rights guard re-routes to the constitutional path ([§3.3](https://glassontin.github.io/civic-kernel/#s3-3 "A vote that infringes fundamental rights cannot pass by a simple majority; it is routed to a higher constitutional threshold.")) |
| T8 | record tampering | [witnessed](README.md#w-witness) append-only logs; any phone detects a rewritten past ([§3.4](https://glassontin.github.io/civic-kernel/#s3-4 "Every action is committed to an append-only log; altering past records breaks cryptographic consistency across all distributed copies.")) |
| T9 | digital exclusion | paper, kiosk, assisted, same protocol, same verifiability ([§4](https://glassontin.github.io/civic-kernel/#s4 "A personal device can submit ballots and audit the transparency log; users without smartphones participate with equal validity through assisted channels.")) |
| T10 | dilution & standards capture | signed conformance [manifests](README.md#w-manifest); [subtraction](README.md#w-subtraction) is legible ([§6](https://glassontin.github.io/civic-kernel/#s6 "Adoption is modular: groups may implement selected components, provided they publish an explicit machine-readable manifest of what is excluded.")) |
| T11 | epistemic gating | no gate on the ballot path; scaffold everything, assess nothing ([§7](https://glassontin.github.io/civic-kernel/#s7 "Participation is not conditioned on competence tests; the system provides clear information without evaluating citizen comprehension.")) |
| T12 | sludge | four verbs, computable eligibility, cited demands, published interaction cost ([§8](https://glassontin.github.io/civic-kernel/#s8 "Administrative burdens are minimized: four standardized verbs, no hidden steps, and every required field must cite its authorizing rule.")) |
| T13 | autoimmune defense | sense at machine speed, respond at constitutional speed ([§9](https://glassontin.github.io/civic-kernel/#s9 "Monitoring operates continuously, but systemic enforcement follows due process; automated tools assist oversight while emergency powers expire automatically.")) |
| T14 | infrastructure denial | kilobyte payloads, delay-tolerant, paper as the zero-bandwidth floor ([§4](https://glassontin.github.io/civic-kernel/#s4 "A personal device can submit ballots and audit the transparency log; users without smartphones participate with equal validity through assisted channels."), [§3.4](https://glassontin.github.io/civic-kernel/#s3-4 "Every action is committed to an append-only log; altering past records breaks cryptographic consistency across all distributed copies.")) |
...
limit [§12](https://glassontin.github.io/civic-kernel/#s12 "The boundaries of the design, including explicit exclusions and unresolved challenges.") concedes), and the `executive-ignores` run breaks on an enforcement gap no
...
## Four services, and no fifth ([§3](https://glassontin.github.io/civic-kernel/#s3 "The system consists of four basic functions: proving eligibility, conducting verifiable votes, enforcing human rights constraints, and maintaining an append-only log."))
...
provide it competitively **and** its absence breaks a [§1](https://glassontin.github.io/civic-kernel/#s1 "The goal: produce decisions that participants accept as legitimate, even when all parties, including the operators, have incentives to manipulate outcomes.") goal. Reputation, deliberation,
...
## Four verbs ([§8](https://glassontin.github.io/civic-kernel/#s8 "Administrative burdens are minimized: four standardized verbs, no hidden steps, and every required field must cite its authorizing rule."))
...
delay-tolerant, deliverable over mesh, radio, or sneakernet when IP dies ([§4](https://glassontin.github.io/civic-kernel/#s4 "A personal device can submit ballots and audit the transparency log; users without smartphones participate with equal validity through assisted channels."), T14).
...
## Two formats, the waist ([§6](https://glassontin.github.io/civic-kernel/#s6 "Adoption is modular: groups may implement selected components, provided they publish an explicit machine-readable manifest of what is excluded."), [`schema/`](schema/))
...
cannot be its own standard ([§6](https://glassontin.github.io/civic-kernel/#s6 "Adoption is modular: groups may implement selected components, provided they publish an explicit machine-readable manifest of what is excluded."), T8, T10).

</details>

## One floor ([§5](https://glassontin.github.io/civic-kernel/#s5 "Rules can be amended through staged and reversible procedures, while fundamental equality of standing remains protected against arbitrary repeal."))
...
pressure pushes toward, elsewhere the kernel guarantees only that the gap is legible ([§5](https://glassontin.github.io/civic-kernel/#s5 "Rules can be amended through staged and reversible procedures, while fundamental equality of standing remains protected against arbitrary repeal.")–[§6](https://glassontin.github.io/civic-kernel/#s6 "Adoption is modular: groups may implement selected components, provided they publish an explicit machine-readable manifest of what is excluded.")).
...
1. **No fifth service.** Nothing opinionated in the core. ([§3](https://glassontin.github.io/civic-kernel/#s3 "The system consists of four basic functions: proving eligibility, conducting verifiable votes, enforcing human rights constraints, and maintaining an append-only log."))
2. **No population database.** Uniqueness without identity, even against the issuer. ([§3.1](https://glassontin.github.io/civic-kernel/#s3-1 "Prove you are on the roster and have not already participated, without revealing your identity or building a personal profile."))
3. **No remote attestation.** Integrity comes from the protocol, or a vendor holds a veto
   over participation. ([§4](https://glassontin.github.io/civic-kernel/#s4 "A personal device can submit ballots and audit the transparency log; users without smartphones participate with equal validity through assisted channels."))
4. **No required device.** The phone is the richest endpoint, never a required one. ([§4](https://glassontin.github.io/civic-kernel/#s4 "A personal device can submit ballots and audit the transparency log; users without smartphones participate with equal validity through assisted channels."), T9)
5. **No token in the citizen's path.** A chain may anchor log heads, notary of last
   resort, never database, never gatekeeper. Participation never costs gas. ([§6](https://glassontin.github.io/civic-kernel/#s6 "Adoption is modular: groups may implement selected components, provided they publish an explicit machine-readable manifest of what is excluded."))
6. **No comprehension gate on the ballot path.** Scaffold everything, assess nothing:
   whoever writes the test owns the electorate. ([§7](https://glassontin.github.io/civic-kernel/#s7 "Participation is not conditioned on competence tests; the system provides clear information without evaluating citizen comprehension."), T11)
7. **Measure the process, never the person.** The same datum is tyranny as an individual
   gate and hygiene as a population signal. ([§7](https://glassontin.github.io/civic-kernel/#s7 "Participation is not conditioned on competence tests; the system provides clear information without evaluating citizen comprehension.")–[§8](https://glassontin.github.io/civic-kernel/#s8 "Administrative burdens are minimized: four standardized verbs, no hidden steps, and every required field must cite its authorizing rule."))
8. **No demand without a cited rule.** A form field that cannot point to the
   machine-readable provision requiring it cannot be asked. ([§8](https://glassontin.github.io/civic-kernel/#s8 "Administrative burdens are minimized: four standardized verbs, no hidden steps, and every required field must cite its authorizing rule."), [§11](https://glassontin.github.io/civic-kernel/#s11 "The same verification infrastructure supports entitlement claims: calculating owed benefits locally, verifying eligibility privately, and requiring reasoned justifications for denials."), T12)
9. **AI holds a microscope, never a pen.** Sense at machine speed, respond at
   constitutional speed; break-glass powers pre-legislated, witnessed, self-expiring. ([§9](https://glassontin.github.io/civic-kernel/#s9 "Monitoring operates continuously, but systemic enforcement follows due process; automated tools assist oversight while emergency powers expire automatically."), T13)
10. **The kernel enforces a membership boundary; it never decides one.** Who counts is
    versioned, witnessed governance. ([§10](https://glassontin.github.io/civic-kernel/#s10 "Enrolment is a public, witnessed ceremony that counts participants rather than recording identities; duplicate registrations and wrongful exclusions are mathematically visible."))
11. **No ministry of truth.** The kernel does not decide truth, host debate, rank
    arguments, moderate speech, or pick outcomes, process integrity only. ([§1](https://glassontin.github.io/civic-kernel/#s1 "The goal: produce decisions that participants accept as legitimate, even when all parties, including the operators, have incentives to manipulate outcomes.")–[§2](https://glassontin.github.io/civic-kernel/#s2 "Identify every failure mode before designing the system: fourteen attacks, each with a designated countermeasure, and two left unaddressed to avoid creating a centralized arbiter of truth."))
...
- **So does the franchise, and it costs anonymity nothing.** A ballot carries no name: it
  ...
  plot-holders. Populations need the constant-size BBS presentation [§13](https://glassontin.github.io/civic-kernel/#s13 "Many underlying components exist in isolation; the goal is integrating them into an accessible, user-verifiable client application.") tracks. The
...
- **T5's challenge and T4's silent re-vote compose, and decided the election.** In the
  ...
  sealed forever.
- **61 scenarios**, each cross-validated against the waist schemas and each anchored to
  documented real-world precedents (168 verified citations, the schema requires them: no
  attack without a cited precedent): 52 strain, 8 break, 1 holds. The one hold is
...
## What is not solved ([§12](https://glassontin.github.io/civic-kernel/#s12 "The boundaries of the design, including explicit exclusions and unresolved challenges."))
